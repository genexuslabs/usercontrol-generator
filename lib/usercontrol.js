...
module.exports = class Usercontrol {
  ...
  constructor(_path) {
    ...
    if (fs.isFileSync(_path)) {
      ...
      parser.parseString(data, function(err, result) {
        if (result.ControlDefinition) isUC = true;
      });
    }
    if (!isUC) throw "This project is not a Usercontrol";
  }
  ...
  static getProjectPath() {
    ...
    if (_pathList.length > 0) return _pathList[0];
    else return "";
  }
  static getPath() {
    let _path = "";
    if (Usercontrol.getProjectPath() != "") {
      let _pathList = fs.listSync(
        path.join(Usercontrol.getProjectPath(), "src"),
        [".control"]
      );
      if (_pathList.length > 0) _path = _pathList[0];
    }
    return _path;
  }
  ...
  static initUC(uc) {
    ...
    utils.scanTree(tplpath, function(err, fileInfo) {
      if (fileInfo.fullpath.indexOf(modpath) != -1)
        // excep node_modules (EMFiles)
        return;
      ...
      if (!fileInfo.isDir) {
        ...
        if (
          fileInfo.basepath == srcpath &&
          /^.*\.(control|xml|js|xsl)$/gi.test(fileInfo.name)
        ) {
          // Usercontrol type template managment
          // _nameRender.common.js
          // _nameRender.grid.js
          // _nameRender.attribute.js
          let copyFile = true;
          if (fileInfo.name.indexOf("__") == 0) {
            copyFile = false;
            if (fileInfo.name.indexOf(uctype) >= 0) {
              copyFile = true;
            }
          }
          ...
          if (copyFile) {
            ...
            data = data.replace(/\{UC.Info\}/gi, uc.info ? "true" : "false");
            if (uc.info == "grid") {
              data = data.replace(/\{UC.Type\}/gi, "grid");
            } else {
              data = data.replace(/\{UC.Type\}/gi, "");
            }

            let plats = "";
            for (let i = 0; i < uc.platforms.length; i++)
              plats +=
                `\t\t<Platform>${uc.platforms[i]}</Platform>` +
                (i == uc.platforms.length - 1 ? "" : "\n");
          ...
          fs.createReadStream(fileInfo.fullpath).pipe(
  ...
  getAttribute(attname) {
    ...
    if (attname == "Version") value = "0.00.01";

    if (
      this.Xml.ControlDefinition &&
      this.Xml.ControlDefinition[attname] &&
      this.Xml.ControlDefinition[attname].length > 0
    ) {
