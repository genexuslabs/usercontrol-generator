...
module.exports = {
	...
	getSuggestions: function(options) {
		if (!this.usercontrol) // Check if is a UC
			return [];

		let schema = "";
		let filename = options.editor.getTitle();

		if (path.extname(filename)==".control")
			schema = "control.xsd";
		else if (filename.toUpperCase()==this.usercontrol.getAttribute("PropertiesDefinition").toUpperCase())
...
//			schema = "xslt.xsd";
		...
		let xpath = xmlutils.getXPath(options.editor.getBuffer(), options.bufferPosition, options.prefix);
	...
	getTagNameCompletions: function(arg) {
		...
		editor = arg.editor, bufferPosition = arg.bufferPosition, prefix = arg.prefix;
		children = xsd.getChildren(xmlutils.getXPath(editor.getBuffer(), bufferPosition, prefix));
	...
	getCloseTagNameCompletion: function(arg) {
		...
		editor = arg.editor, bufferPosition = arg.bufferPosition, prefix = arg.prefix;
		parentTag = xmlutils.getXPath(editor.getBuffer(), bufferPosition, prefix, 1);
	...
	getValuesCompletions: function(arg) {
		var bufferPosition, children, editor, prefix;
		editor = arg.editor, bufferPosition = arg.bufferPosition, prefix = arg.prefix;
		children = xsd.getValues(xmlutils.getXPath(editor.getBuffer(), bufferPosition, ''));
