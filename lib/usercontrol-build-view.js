...
export default class UsercontrolBuildView extends BaseForm {
  ...
  attach(env) {
    ...
    try {
      ...
      if (this.environment == "debug") setTimeout(() => this.confirm(), 500);
    } catch (ex) {
  ...
  runApm(task) {
    ...
    Usercontrol.exec(
      command,
      args,
      Usercontrol.getProjectPath(),
      out => this.gulpOut(out),
      out => this.gulpErr(out),
      function(code) {
        this.block(false);
  ...
  runGulp(task) {
    if (
      !Utils.pathExists(path.join(Usercontrol.getProjectPath(), "node_modules"))
    ) {
      Utils.checkInternet(
        function(err) {
          if (err) {
    ...
    args = [
      ...
      "--color",
      "--target=" + task,
      "--ucname=" + this.GXUC.getAttribute("Name")
    ];
  ...
  updateGx() {
    ...
    command = path.join(
      Usercontrol.getPackageBasePath(),
      "helpers",
      "elevate",
      "elevate",
      "bin",
    ...
    if (fs.existsSync(conf.gxpath)) {
      ...
      Usercontrol.exec(
        ...
        out => this.gulpErr(out),
        code => {
          this.gulpExit(code, true);
        }
      );
    } else {
      this.writeOutput(
        `GeneXus doesn't exists in the specified path (${conf.gxpath})`
      );
    }
  }
}
